...
#define Max_No_of_Frames 32
using namespace std;
class Head
{
    private:

    int Data_Set[Max_No_of_Frames]={23,24,35,335,46,416,464,53,524,37,76,7,6,787,8,78,67,64,
                                    65,75,742,727,37,27,272,72,727,466,174,321,62,22};
    ...
    void Scan()
    {
        ...
        No_of_bits_to_send = Pipeline_Size;
    ...
    void Event_Send(int Frames_Send)
    {
        ...
        if (Negative_Ack!=-1)
        {
            Pipeline[k]=Negative_Ack;
            cout<<"\nFrame No. "<<Pipeline[k]<<" with data "<<Data_Set[Pipeline[k]]<<" is resent.";
            k++;
        }
        for (i=Bits_Sent ; i<Bits_Sent+No_of_bits_to_send ; )
        {
            Pipeline[k]=Data[i++];
            cout<<"\nFrame No. "<<Pipeline[k++]<<" with data "<<Data_Set[i]<<" is sent.";
        }
    ...
    void  Event_Receive(int Frames_Received)
    {
         ...
         bool cont;
         cout <<"\n\nReceiver Message :\n";
         Trans_Error = rand() % 4; // probability of transmission error = 1/5
         ...
         if(Trans_Error!=0)
         {
            ...
            {
                if (Bits_Received == Pipeline[i] || Bits_Received == Negative_Ack)
                {
                    cout <<"\nFrame No. "<<Pipeline[i]<<" with data "<<Data_Set[i]<<" is received correctly.";
                    ++Bits_Received;
                    //Bits_Received%=Frames_Received;
                }
                else
                    cout << "\nDuplicate Frame No. "<<Pipeline[i]<<" with data "<<Data_Set[i]<<" is received thus discarded.";
            }
            Ack_Lost_Prob = rand() % 5;
            ...
            {
                Ack_Lost_Prob = rand() % Pipeline_Size;
                cout << "\nAcknowledgment "<<Pipeline[Ack_Lost_Prob]<<" is lost.";
                //Bits_Sent = Bits_Received = Pipeline[Ack_Lost_Prob];
                Negative_Ack=Pipeline[Ack_Lost_Prob];
            }
            //else
         ...
         else
         {
            ...
            for (int i = 0 ; i < Lost_Index ; i++)
            {
                ...
                {
                    cout<<"\nFrame No. "<<Pipeline[i]<<" with data "<<Data_Set[i]<<" is recieved correctly.";
                    ++Bits_Received;
                    Bits_Received%=Frames_Received;
                }
                else
...
int  main()
{
    Head Selective_Repeat;
    Selective_Repeat.Negative_Ack=-1;
    Selective_Repeat.Scan();
}
